...
/**
 * This Field Finds the Installed Stacks & Adds a List Field for Each
 * Which Allows you to Select a File Type Parameter for Each Stack.
 *
 * @todo 	Merge This File With "stacklocations" as They Are Virtually the Same. Called "stackfield"
 * 			Rather Than Extending the Desired Field Perhaps Create a New JForm Instance for Each Field?
 * @todo 	This Field Does Not Support the Asterisk Wildcard Even Though it Extends the Multiple List!
 *        	Because the Default of '*' is Caught in the setup(); Whereas we set it in the getInput();
 */
class ArkFormFieldStackFileTypes extends JFormFieldGroupedList
{
	/**
	 * The form field type.
	 *
	 * @var		string
	 */
	protected $type = 'stackfiletypes';

	/**
	 * Custom Param (like $translateLabel & $translateHint) to Decide Whether to Tranlate the Data Attr's.
	 *
	 * @var		bool
	 */
	protected $translateAttributes = false;
	...
	protected function getInput()
	{
		...
		foreach( HelperStack::get() as $stack => $data )
		{
			...
			// Get Details
			$default 	= ( isset( $vals[$stack] ) && count( $vals[$stack] ) ) ? $vals[$stack] : $data->params->get( $name );
